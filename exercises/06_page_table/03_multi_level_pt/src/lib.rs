...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // 先读取 PTE，释放借用
        let pte2 = {
            let node2 = self.nodes.get(&self.root_ppn).unwrap();
            node2.entries[vpn_2]
        };

        let ppn1 = if pte2 & PTE_V == 0 {
            // 分配新节点（需要 &mut self，所以必须先释放之前的借用）
            let new_ppn = self.alloc_node();
            // 重新借用，写入 PTE
            let node2 = self.nodes.get_mut(&self.root_ppn).unwrap();
            node2.entries[vpn_2] = (new_ppn << 10) | PTE_V;
            new_ppn
        } else {
            (pte2 >> 10) & 0xFFFFFFFFFFF
        };

        // Level 1: 获取 ppn0
        let pte1 = {
            let node1 = self.nodes.get(&ppn1).unwrap();
            node1.entries[vpn_1]
        };

        let ppn0 = if pte1 & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            let node1 = self.nodes.get_mut(&ppn1).unwrap();
            node1.entries[vpn_1] = (new_ppn << 10) | PTE_V;
            new_ppn
        } else {
            (pte1 >> 10) & 0xFFFFFFFFFFF
        };

        //  Level 0: 写入叶子 PTE
        // 物理页号写入 PTE: (pa >> 12) 左移 10 位，加上 flags 和 V 标志
        let node0 = self.nodes.get_mut(&ppn0).unwrap();
        node0.entries[vpn_0] = ((pa >> 12) << 10) | flags | PTE_V;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        if pte2 & (PTE_R | PTE_W | PTE_X) != 0 {
            let ppn = (pte2 >> 10) & 0xFFFFFFFFFFF;
            let offset = va & 0x1FFFFF; // 21 bits for 2MB superpage at level 2
            return TranslateResult::Ok((ppn << 12) | offset);
        }
        let ppn1 = (pte2 >> 10) & 0xFFFFFFFFFFF;

        // Level 1
        let pte1 = self.nodes.get(&ppn1).unwrap().entries[vpn1];
        if pte1 & PTE_V == 0 {
            return TranslateResult::PageFault;
        }
        // 检查是否是叶子 PTE（2MB 大页在 level 1）
        if pte1 & (PTE_R | PTE_W | PTE_X) != 0 {
            let ppn = (pte1 >> 10) & 0xFFFFFFFFFFF;
            let offset = va & 0x1FFFFF; // 21 bits for 2MB superpage
            return TranslateResult::Ok((ppn << 12) | offset);
        }
        let ppn0 = (pte1 >> 10) & 0xFFFFFFFFFFF;

        // Level 0（普通 4KB 页）
        let pte0 = self.nodes.get(&ppn0).unwrap().entries[vpn0];
        if pte0 & PTE_V == 0 {
            return TranslateResult::PageFault;
        }
        ...
        224 -          └────────────────────┴────────────────┘
        225 -         */
        let offset = va & 0xFFF; // 12 bits for 4KB page
        let ppn = (pte0 >> 10) & 0xFFFFFFFFFFF;
        return TranslateResult::Ok((ppn << 12) | offset);
    }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        //获取第一层的页表
        let pte2 = {
            let page_table_node = self.nodes.get(&current_ppn).unwrap();
            let pte = page_table_node.entries[vpn2];
            pte
        };
        ...
        let ppn1 = if pte2 & PTE_V == 0 {
            ...
            let new_ppn = self.alloc_node();
            let node2 = self.nodes.get_mut(&self.root_ppn).unwrap();
            node2.entries[vpn2] = (new_ppn << 10) | PTE_V;
            new_ppn
        } else {
            (pte2 >> 10) & 0xFFFFFFFFFFF
        };
        //往ppn1中存入大页表
        let page_table_node = self.nodes.get_mut(&ppn1).unwrap();
        //构建pa
        page_table_node.entries[vpn1] = ((pa >> 12) << 10) | flags | PTE_V;
    }
}
