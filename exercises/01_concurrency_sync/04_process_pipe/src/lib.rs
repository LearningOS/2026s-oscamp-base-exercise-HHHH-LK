...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    child.wait().unwrap(); // Wait for child to exit (optional)
    output
}
...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    let status = Command::new("sh").args(["-c", command]).status().expect("Failed to execute command");
    // TODO: Return exit code
    status.code().unwrap_or(-1) // Return -1 if process was terminated by signal or other non‑normal exit
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    command.args([pattern]).stdin(Stdio::piped()).stdout(Stdio::piped());
    //创建相对应的子进程
    let mut child = command.spawn().expect("Failed to spawn grep process");
    {   
        let mut stdin_pipe=child.stdin.take().unwrap();
        stdin_pipe.write_all(input.as_bytes()).expect("Failed to write to grep stdin");
        drop(stdin_pipe); // Close stdin to signal EOF to grep}
    }
    // Read grep's output
    let mut stdout_pipe = child.stdout.take().unwrap();
    let mut output = String::new();
    stdout_pipe.read_to_string(&mut output).expect("Failed to read from grep stdout");
