...
/// Sum two vectors in parallelv, returning a tuple of two sums.
///
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    thread::scope(|s|{
        let thread_1=s.spawn(move || a.iter().sum::<i32>());
        let thread_2=s.spawn(move || b.iter().sum::<i32>());
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    ...
    thread::Builder::new().name("sleeper".into()).stack_size(32*1024).spawn(move ||{
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    thread::scope(|s|{
        let thread_1=s.spawn(|| a.iter().sum::<i32>());
        let thread_2=s.spawn(|| b.iter().sum::<i32>());
        (thread_1.join().unwrap(), thread_2.join().unwrap())
    })
...
pub fn handle_panic(value: i32, should_panic: bool) -> Result<i32, ()> {
    ...
    //只有调用join后进程才会等待线程结束并获取结果，如果线程发生panic，join会返回Err。
  match handler.join() {
        Ok(v) => Ok(v),
        Err(_) => Err(()),
    }
