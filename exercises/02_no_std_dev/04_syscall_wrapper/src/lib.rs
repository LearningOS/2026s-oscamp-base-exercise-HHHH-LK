...
pub fn riscv64_abi() -> SyscallABI {
    SyscallABI {
        arch: "riscv64",
        instruction: "ecall",
        // 1. 系统调用号放在 a7 (对应物理寄存器 x17)
        id_reg: "a7", 
        
        // 2. 返回值放在 a0 (对应物理寄存器 x10)，绝对不是 x0!
        ret_reg: "a0", 
        
        // 3. 参数依次放在 a0, a1, a2, a3, a4, a5
        // 注意：a0 既用于传入第一个参数，也用于接收返回值
        arg_regs: &["a0", "a1", "a2", "a3", "a4", "a5"],
        
        // 4. ecall 指令本身根据 ABI 不会额外破坏 t0-t6 等临时寄存器
        // 测试报错提示你声明了多余的 clobbered 寄存器，这里改为空
        clobbered: &[], 
        
        // 系统调用号 (Linux RISC-V 标准)
        sys_read: 63,
        sys_write: 64,
        sys_close: 57,
        sys_exit: 93,
...
pub fn sys_write(fd: usize, buf: &[u8]) -> isize {
    ...
    unsafe {
        let ret = syscall3(64, fd, buf.as_ptr() as usize, buf.len());
        return ret;
    };
}
...
pub fn sys_read(fd: usize, buf: &mut [u8]) -> isize {
    ...
    unsafe {
        let ret = syscall3(63, fd, buf.as_ptr() as usize, buf.len());
        return ret;
    }
}
...
pub fn sys_exit(code: i32) -> ! {
    ...
    unsafe {
        syscall3(93, code.try_into().unwrap(), 0, 0);
    }
    panic!("Exit failed !");
}
